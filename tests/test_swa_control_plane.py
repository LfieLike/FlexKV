...
def test_put_builds_full_plus_swa_store_chain():
    ...
    hit, slot, _node = eng.cpu_cache_engine._resolve_swa_hit(
...
def test_get_builds_full_plus_swa_load_chain():
    ...
    hit, slot, node = eng.cpu_cache_engine._resolve_swa_hit(
...
def test_get_ssd_staging_when_only_ssd_has_swa():
    ...
    cpu_hit, _s, _n = eng.cpu_cache_engine._resolve_swa_hit(
        seq, upper_bound_blocks=4)
    assert cpu_hit == 0, "precondition: CPU SWA must be gone"
    ssd_hit, _s2, _n2 = eng.ssd_cache_engine._resolve_swa_hit(
        seq, upper_bound_blocks=4)
...
def test_multitier_match_promotes_swa_in_each_tier():
    ...
    for name, engine in (("cpu", eng.cpu_cache_engine), ("ssd", eng.ssd_cache_engine)):
        a_hit, _sa, _na = engine._resolve_swa_hit(seq_a, upper_bound_blocks=4)
        b_hit, _sb, _nb = engine._resolve_swa_hit(seq_b, upper_bound_blocks=4)
